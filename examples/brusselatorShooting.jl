...
probSh = ShootingProblem(Fbru, par_hopf,
	prob, Rodas4P(),
	[orbitguess_f2[:,ii] for ii=1:dM:M]; abstol = 1e-10, reltol = 1e-8, parallel = false)
...
opts_po_cont = ContinuationPar(dsmax = 0.03, ds= 0.01, pMax = 2.5, maxSteps = 100, newtonOptions = (@set optn_po.tol = 1e-7), nev = 15, precisionStability = 1e-3, detectBifurcation = 3, plotEveryStep = 2, saveSolEveryStep = 2)
...
probHPsh = PoincareShootingProblem(Fbru, par_hopf, prob, Rodas4P(), normals, centers; abstol = 1e-10, reltol = 1e-8, parallel = false, δ = 1e-8)
