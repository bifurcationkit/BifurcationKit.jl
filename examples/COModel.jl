...
opts_br = ContinuationPar(pMin = 0.5, pMax = 2.3, ds = 0.002, dsmax = 0.01, nInversion = 10, detectBifurcation = 3, maxBisectionSteps = 25, nev = 3, maxSteps = 20000, saveSolEveryStep = 1)
...
sn, = newton(jet[1:2]..., br, 2; options = opts_br.newtonOptions)
...
sn_codim2, = continuationFold(jet[1:2]..., br, 2, (@lens _.k), ContinuationPar(opts_br, pMax = 3.2, pMin = 0., detectBifurcation = 1, dsmin=1e-5, ds = -0.001, dsmax = 0.05, nInversion = 6, detectEvent = 2, detectFold = false) ; plot = false,
	verbosity = 3,
	normC = norminf,
...
cp = computeNormalForm(jet..., sn_codim2, 2)
	display(cp.x0)
	show(cp)
...
# case of BT point
bt = BK.bogdanovTakensNormalForm(jet..., sn_codim2, 1; verbose = true, autodiff = false)
	display(bt.x0)
	show(bt)
...
@test isapprox(bt.nfsupp.a1, -0.8618; rtol = 1e-4)
@test isapprox(bt.nfsupp.b1, -7.1176; rtol = 1e-4)

hp_codim2, = continuation(jet[1:2]..., br, 1, (@lens _.k), ContinuationPar(opts_br, pMin = 0., pMax = 2.8, detectBifurcation = 1, ds = -0.0001, dsmax = 0.02, dsmin = 1e-4, nInversion = 12, detectLoop = true, maxSteps = 250, maxBisectionSteps = 35 ) ;
...
gh = computeNormalForm(jet..., hp_codim2, 2; verbose = true)
	display(gh.x0)
	show(gh)
