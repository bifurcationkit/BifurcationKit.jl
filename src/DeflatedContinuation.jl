...
function continuation(F, J, par, lens::Lens, contParams::ContinuationPar, defOp::DeflationOperator;
	...
	function getNewSolution(_st::DCState, _p::Real, _idb)
		newton(F, J, perturbSolution(getx(_st), _p, _idb), set(par, lens, _p), setproperties(optnewton; maxIter = maxIterDefOp), deflationOp; normN = normN, callback = callbackN, fromDeflatedNewton = true)
	end
	...
	showplot && display(plot(branches..., label = ""))
