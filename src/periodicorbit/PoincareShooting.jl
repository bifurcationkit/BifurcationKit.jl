...
function PoincareShootingProblem(F, p,
			...
			δ = 1e-8, interp_points = 50, kwargs...)
...
function PoincareShootingProblem(F, p,
	...
	cb = VectorContinuousCallback(pSection, affect!, hyp.M; interp_points = interp_points, affect_neg! = nothing)
	return HyperplanePoincareShootingProblem(
			PoincareShootingProblem(flow = Flow(F, p, prob1, alg1, prob2, alg2; callback = cb, kwargs...), M = hyp.M, section = hyp, δ = δ))
end
...
function diffPoincareMap(hpsh::HyperplanePoincareShootingProblem, x, dx, ii::Int)
	sh = hpsh.psh
