...
import BifurcationKit: potrap_scheme!, residual!
...
function bvp_residual(bvp::DiscretizedBVP{<:BVPModel, <:Trap}, X, p)
    ...
    # M time points span [t0, tf] via M-1 intervals, each of length δT/(M-1).
    ...
    h_step = δT / (M - 1)   # BVP: M points → M-1 intervals (NOT δT/M which is cyclic PO convention)
    tmp = similar(X, n)
    residual!(po_trap.prob_vf, tmp, @view(Xc[:, 1]), p)  # seed: F(u_1)

    for i in 1:M-1
        potrap_scheme!(po_trap, @view(outc[:, i]), @view(Xc[:, i+1]), @view(Xc[:, i]), p, h_step/2, tmp)
        # outc[:, i] = u_{i+1} - u_i - (h_step/2)*(F(u_i) + F(u_{i+1}))
