...

{
    ...
    "category": "section",
    "text": "We were inspired by pde2path. One can also mention the venerable AUTO, or also, XPPAUT, MATCONT and COCO or Trilinos. Most continuation softwares are listed on DSWeb. There is also this MATLAB continuation code by D. Avitabile.In Julia, we have for now a wrapper to PyDSTools, and also Bifurcations.jl.One design choice is that we do not require u to be a subtype of an AbstractArray as this would forbid the use of spectral methods like the one from ApproxFun.jl. So far, our implementation does not allow this for Hopf continuation and computation of periodic orbits. It will be improved later."
},
...

{
    ...
    "title": "Linear solvers",
    "category": "section",
    "text": "The linear solvers are subtypes of AbstractLinearSolver. Basically, one must provide a way of inverting the Jacobian J or computing J \\ x.Here is an example of the simplest of them (see src/LinearSolver.jl) to give you an idea, the backslash operator:struct DefaultLS <: AbstractLinearSolver end\n\nfunction (l::DefaultLS)(J, rhs)\n	return J \\ rhs, true, 1\nendNote that for newton to work, you must return 3 arguments. The first one is the result, the second one is whether the computation was successful and the third is the number of iterations required to perform the computation.You can call it like (and it will be called like this in newton)ls = DefaultLS()\nls(rand(2,2), rand(2))You can instead define struct myLinearSolver <: AbstractLinearSolver end and write (l::myLinearSolver)(J, x) where this function would implement GMRES or whatever you prefer."
},

{
    "location": "linearsolver/#List-of-implemented-solvers-1",
    "page": "Linear Solvers",
    "title": "List of implemented solvers",
    "category": "section",
    "text": "GMRES from IterativeSolvers.jl. You can call it via linsolver = GMRES_IterativeSolvers() and pass appropriate options.\nGMRES from KrylovKit.jl. You can call it via linsolver = GMRES_KrylovKit{Float64}() and pass appropriate options."
},
...

{
    ...
    "category": "section",
    "text": "The bifurcations are detected during a call to br, _ = continuation(F, J, u0, p0::Real, contParams::ContinuationPar;kwrags...) by turning on one of the following flags:contParams.computeEigenValues = true to trigger the computation of the eigenvalues of the jacobian. It automatically turn on the detection of bifurcation points.\n`contParams.detect_bifurcation = true which also turns on contParams.computeEigenValues = trueThe located bifurcation points are then returned in br.bifpoint. Note that this points are only approximate bifurcation points. They need to be refined with the methods described here after."
},

{
    ...
    "title": "Eigensolver",
    "category": "section",
    "text": "The user must provide an eigensolver by setting NewtonOptions.eigsolve where NewtonOptions is located in the parameter ::ContinuationPar passed to continuation. See src/Newton.jl for more information on the structure of the options passed to newton and continuation.The eigensolver is highly problem dependent and this is why the user should implement / parametrize its own eigensolver through the abstract type AbstractEigenSolver or select one among the provided like DefaultEig(), eig_IterativeSolvers(), eig_KrylovKit. See src/EigSolver.jl."
},

{
    ...
    "category": "section",
    "text": "The detection of Fold point is done by monitoring  the monotonicity of the parameter.The detection is triggered by setting detect_fold = true in the parameter ::ContinuationPar passed to continuation. When a Fold is detected, a point is added to br.bifpoint allowing for later refinement using the function newtonFold."
},
...

{
    ...
    "title": "Functions",
    "category": "section",
    "text": "newtonFold(F, J, Jt, foldpointguess::BorderedArray{vectype, T}, eigenvec, options::NewtonPar; normN = norm) where {T,vectype}newtonFoldnewtonHopfcontinuationFoldcontinuationHopf"
},
...
{
    "location": "library/#PseudoArcLengthContinuation.DeflationOperator",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.DeflationOperator",
    "category": "type",
    "text": "DeflationOperator. It is used to describe the following situation. Assume you want to solve F(x)=0 with a Newton algorithm but you want to avoid the process to return some already known solutions roots_i. You can use DeflationOperator to define a function M(u) used to find, with Newton iterations, the zeros of the following function F(u)  Π_i(dot(u - roots_i u - roots_i)^p + shift) = F(u)  M(u). The fields of the struct DeflationOperator are as follows:\n\npower p\ndot function, this function has to be bilinear and symmetric for the linear solver\nshift\nroots\n\nThe deflation operator is is M(u) = frac1prod_i=1^n_roots(shift + norm(u-roots_i)^p) where nrm(u) = dot(uu).\n\n\n\n\n\n"
},

{
    "location": "library/#PseudoArcLengthContinuation.newtonDeflated-Union{Tuple{vectype}, Tuple{Tf}, Tuple{T}, Tuple{Any,Any,vectype,NewtonPar{T,S,E} where E<:AbstractEigenSolver where S<:AbstractLinearSolver,DeflationOperator{T,Tf,vectype}}} where vectype where Tf where T",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.newtonDeflated",
    "category": "method",
    "text": "newtonDeflated(F, J, x0, options:: NewtonPar{T}, defOp::DeflationOperator{T, vectype}; kwargs)\n\nThis is the deflated version of the Newton Solver for F(x) = 0 with Jacobian J. It penalises the roots saved in defOp.roots. The other arguments are as for newton.\n\nSimplified calls are provided, for example when J is not passed. It then computed with finite differences.\n\nOutput:\n\nsolution:\nhistory of residuals\nflag of convergence\nnumber of iterations\n\n\n\n\n\n"
},

{
    "location": "library/#Newton-with-deflation-1",
    "page": "Library",
    "title": "Newton with deflation",
    "category": "section",
    "text": "DeflationOperatornewtonDeflated(Fhandle, Jhandle, x0::vectype, options:: NewtonPar{T}, defOp::DeflationOperator{T, Tf, vectype}; kwargs...) where {T, Tf, vectype}"
},

{
    "location": "library/#PseudoArcLengthContinuation.newtonFold-Union{Tuple{vectype}, Tuple{T}, Tuple{Any,Any,Any,Any,BorderedArray{vectype,T},Any,NewtonPar}} where vectype where T",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.newtonFold",
    "category": "method",
    "text": "This function turns an initial guess for a Fold point into a solution to the Fold problem based on a Minimally Augmented formulation. The arguments are as follows\n\nF   = (x, p) -> F(x, p) where p is the parameter associated to the Fold point\ndF  = (x, p) -> d_xF(x, p) associated jacobian\ndFt = (x, p) -> transpose(d_xF(x, p)) associated jacobian, it should be implemented in an efficient manner. For matrix-free methods, tranpose is not readily available.\nd2F = (x, p, v1, v2) ->  d2F(x, p, v1, v2) a bilinear operator representing the hessian of F. It has to provide an expression for d2F(x,p)[v1,v2].\nfoldpointguess initial guess (x0, p0) for the Fold point. It should be a BorderedArray as given by the function FoldPoint\neigenvec guess for the 0 eigenvector\noptions::NewtonPar\n\n\n\n\n\n"
...
{
    "location": "library/#PseudoArcLengthContinuation.newtonFold-Tuple{Any,Any,Any,Any,ContResult,Int64,NewtonPar}",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.newtonFold",
    "category": "method",
    "text": "Simplified call to refine an initial guess for a Fold point. More precisely, the call is as follows\n\n`newtonFold(F, J, Jt, br::ContResult, index::Int64, options::NewtonPar)`\n\nor\n\n`newtonFold(F, J, Jt, d2F, br::ContResult, index::Int64, options::NewtonPar)`\n\nwhether the Hessian d2F is known analytically or not. The parameters / options are as usual except that you have to pass the branch br from the result of a call to continuation with detection of bifurcations enabled and index is the index of bifurcation point in br you want to refine.\n\n\n\n\n\n"
},

{
    ...
    "category": "section",
    "text": "newtonFold(F, J, Jt, d2F, foldpointguess::BorderedArray{vectype, T}, eigenvec, options::NewtonPar; normN = norm, d2F_is_known = true ) where {T,vectype}newtonFold(F, J, Jt, d2F, br::ContResult, ind_fold::Int64, options::NewtonPar;kwargs...)"
},
...
{
    "location": "library/#PseudoArcLengthContinuation.continuationFold-Union{Tuple{vectype}, Tuple{T}, Tuple{Any,Any,Any,Any,BorderedArray{vectype,T},T,Any,ContinuationPar}} where vectype where T",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.continuationFold",
    "category": "method",
    "text": "Codim 2 continuation of Fold points. This function turns an initial guess for a Fold point into a curve of Fold points based on a Minimally Augmented formulation. The arguments are as follows\n\nF = (x, p1, p2) ->	F(x, p1, p2) where p is the parameter associated to the Fold point\nJ = (x, p1, p2) -> d_xF(x, p1, p2) associated jacobian\nJt = (x, p1, p2) -> transpose(d_xF(x, p1, p2)) associated jacobian\nd2F = (x, p1, p2, v1, v2) -> d2F(x, p1, p2, v1, v2) this is the hessian of F computed at (x, p1, p2) and evaluated at (v1, v2).\nfoldpointguess initial guess (x0, p10) for the Fold point. It should be a BorderedArray as given by the function FoldPoint\np2 parameter p2 for which foldpointguess is a good guess\neigenvec guess for the 0 eigenvector at p1_0\noptions::NewtonPar\n\n\n\n\n\n"
...
{
    "location": "library/#PseudoArcLengthContinuation.continuationFold-Union{Tuple{vectype}, Tuple{T}, Tuple{Any,Any,Any,BorderedArray{vectype,T},T,Any,ContinuationPar}} where vectype where T",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.continuationFold",
    "category": "method",
    "text": "codim 2 continuation of Fold points. This function turns an initial guess for a Fold point into a curve of Fold points based on a Minimally Augmented formulation. The arguments are as follows\n\nF = (x, p1, p2) -> F(x, p1, p2) where p is the parameter associated to the Fold point\nJ = (x, p1, p2) -> d_xF(x, p1, p2) associated jacobian\nfoldpointguess initial guess (x0, p10) for the Fold point. It should be a BorderedArray as given by the function FoldPoint\np2 parameter p2 for which foldpointguess is a good guess\neigenvec guess for the 0 eigenvector at p1_0\noptions::NewtonPar\n\nwarning: Hessian\n\n\nThe hessian of `F` in this case is computed with Finite differences. This can be slow for many variables, e.g. ~1e6\n\n\n\n\n\n"
...
{
    "location": "library/#PseudoArcLengthContinuation.continuationFold-Tuple{Any,Any,Any,Any,ContResult,Int64,Real,ContinuationPar}",
    "page": "Library",
    "title": "PseudoArcLengthContinuation.continuationFold",
    "category": "method",
...

{
    ...
    "category": "section",
    "text": "continuationFold(F, J, Jt, d2F, foldpointguess::BorderedArray{vectype, T}, p2_0::T, eigenvec, options_cont::ContinuationPar ; d2F_is_known = true, kwargs...) where {T,vectype}continuationFold(F, J, Jt, foldpointguess::BorderedArray{vectype, T}, p2_0::T, eigenvec, options_cont::ContinuationPar ;kwargs...) where {T,vectype}continuationFold(F, J, Jt, d2F, br::ContResult, ind_fold::Int64, p2_0::Real, options_cont::ContinuationPar ; kwargs...)"
},
