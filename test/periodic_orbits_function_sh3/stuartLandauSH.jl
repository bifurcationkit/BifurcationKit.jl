...
M = 1
dM = 1
_pb = Shooting(prob, Vern9(), 1, section; abstol = 1e-10, reltol=1e-9)
...
_pb_par = Shooting(prob, Vern9(), 1, section; abstol=1e-10, reltol=1e-9, parallel = true)
...
_sh = Shooting(prob, Vern9(), [initpo[1:end-1]]; abstol=1e-10, reltol=1e-9, lens = (@optic _.r))
...
_sh2 = Shooting(prob, Vern9(), probMono, Vern9(), [initpo[1:end-1]]; abstol = 1e-10, reltol = 1e-9)
...
br_pok2_s2 = continuation(br, 1, (@set opts_po_cont.newton_options.verbose = false), Shooting(2, prob, Vern9();  abstol = 1e-10, reltol = 1e-9, lens = (@optic _.r)); normC = norminf)
...
let
    initpo = [0.13, 0., 6.]
    _sh = Shooting(prob, Vern9(), [initpo[1:end-1],initpo[1:end-1],initpo[1:end-1]]; abstol =1e-10, reltol=1e-9)
...
let
    initpo = [0.13, 0, 6]
    _sh = Shooting(prob, Vern9(), [initpo[1:end-1],initpo[1:end-1],initpo[1:end-1]]; abstol =1e-10, reltol=1e-9, parallel = true)
    initpo = [0.13, 0, 0, 0.13, 0, 0.13 , 6.3]
    res = BK.po_residual(_sh, initpo, par_hopf)
    res = BK.po_jvp(_sh, initpo, par_hopf, initpo)
    # test the jacobian of the functional
    ...
    _sh2_par = Shooting(prob, Vern9(), probMono, Vern9(), [initpo[1:end-1], initpo[1:end-1], initpo[1:end-1]]; abstol = 1e-10, reltol = 1e-9, parallel = true)
...
_psh = PoincareShooting(2, prob, Vern9(), probMono, Vern9(); abstol=1e-10, reltol=1e-9, jacobian = BK.AutoDiffDenseAnalytical())
@test _psh.par == _psh.flow.prob1.p

_psh = PoincareShooting(2, prob, Vern9(); rtol = abstol=1e-10, reltol=1e-9, jacobian = BK.AutoDiffDenseAnalytical())
@test _psh.par == _psh.flow.prob.p

_psh = PoincareShooting(prob, Vern9(),
        probMono, Vern9(),
...
_psh = PoincareShooting(prob, Vern9(), normals, centers; abstol=1e-11, reltol=1e-9, lens = (@optic _.r), jacobian = BK.AutoDiffDenseAnalytical())
# version with analytical jacobian
_psh2 = PoincareShooting(prob, Vern9(), normals, centers; abstol=1e-11, reltol=1e-9, δ = 0, lens = (@optic _.r), jacobian = BK.AutoDiffDenseAnalytical())
...
br_psh = continuation(br, 1, (@set opts_po_cont.ds = 0.005), PoincareShooting(2, prob, Vern9(); abstol=1e-10, reltol=1e-9, parallel = true, lens = @optic _.r); normC = norminf)
...
br_sh = continuation(br, 1, ContinuationPar(opts_po_cont; ds = 0.005, save_sol_every_step = 1), Shooting(2, prob, Vern9(); abstol=1e-10, reltol=1e-9, lens = @optic _.r); normC = norminf)
@test BK.getprob(br_psh) isa BK.PeriodicOrbitFunctionalSh
@test br_psh.period[1] ≈ 2pi rtol = 1e-7
...
for M in (1,2), jacobianPO in (BK.AutoDiffMF(), BK.MatrixFree(), BK.AutoDiffDenseAnalytical(), BK.FiniteDifferences())
    ...
    local br_ssh = continuation(br, 1, (@set opts_po_cont.ds = 0.005),
            Shooting(M, prob, Vern9(); abstol=1e-10, reltol=1e-9, parallel = _parallel, jacobian = jacobianPO, update_section_every_step = 2); 
            normC = norminf,
