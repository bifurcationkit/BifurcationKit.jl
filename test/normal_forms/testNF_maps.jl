...
let
    ...
    for b in (0, 0.21), saveev in (true, false), jettype in (BK.AutoDiff(), BK.FiniteDifferences())
...
let
    ...
    for jettype in (BK.AutoDiff(), BK.FiniteDifferences())
...
let
    ...
    for jettype in (BK.AutoDiff(), BK.FiniteDifferences())
        prob_ns = BK.BifurcationProblem((x, p) -> Fns(x, p) .- x, 0.01rand(2), pars_ns, (@optic _.μ);
                R01 = jettype, R11 = jettype)
        opt_newton = NewtonPar(tol = 1e-9, max_iterations = 20)
        opts_br = ContinuationPar(dsmin = 0.001, dsmax = 0.05, ds = 0.01, p_max = 0.4, p_min = -0.5, detect_bifurcation = 3, nev = 2, newton_options = opt_newton, max_steps = 100, n_inversion = 6, tol_bisection_eigenvalue = 1e-8, dsmin_bisection = 1e-9)
        ...
        nf = BK.neimark_sacker_normal_form(prob, br, 1; nev = 2, verbose = true, detailed = true)
        @test nf.nf.a ≈ pars_ns.a atol=1e-3
        @test nf.nf.b ≈ pars_ns.c3 atol=1e-2
        show(nf)
        BK.type(nf)
