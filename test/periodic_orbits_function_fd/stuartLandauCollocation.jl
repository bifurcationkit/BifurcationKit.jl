...
let
    ...
    Collocation(10, 2) |> BK.get_mesh_size
    BK.get_Ls(coll)
    show(coll)
...
@views function phaseCond(pb::Collocation, u, v)
    𝒯 = eltype(u)
    phase = zero(𝒯)
...
let
    for Ntst in 2:10:100
        # @info "Ntst" Ntst
        coll = Collocation(Ntst, 10, prob_vf = probsl, N = 1)
...
let
    coll = Collocation(22, 10, prob_vf = probsl, N = 1)
...
let
    ...
    J = (Jw) |> copy
    @test BK._eig_floquet_coll((J),N,m,Ntst,nev)[1] ≈ BK._eig_floquet_coll(sparse(J),N,m,Ntst,nev)[1] atol=1e-10
    @test BK._eig_floquet_coll_small_n((J),N,m,Ntst,nev,BK.COPCACHE(BK.get_discretization(BK.getprob(br_po)), Val(0)))[1] ≈ 
          BK._eig_floquet_coll((J),N,m,Ntst,2)[1] atol=1e-10
    @test BK._eig_floquet_coll_small_n((J),N,m,Ntst,nev,BK.COPCACHE(BK.get_discretization(BK.getprob(br_po)), Val(0)))[1] ≈ 
                FloquetGEV(DefaultEig(),size(J,1)-1,2)(BK.get_discretization(BK.getprob(br_po)),Jw,2)[1]
end
####################################################################################################
    ...
    for jacPO in (BK.DenseAnalytical(), BK.AutoDiffDense(), BK.FullSparse(), BK.DenseAnalyticalInplace(), ), use_nf in (true, false)
        ...
        for lspo in (BK.MatrixBLS(), BK.COPBLS())
            ...
            br_po_adapt = continuation(br, 1, _cont_po,
                Collocation(20, 5; jacobian = jacPO, meshadapt = true);
                δp = 0.1,
                use_normal_form = use_nf,
                linear_algo = lspo,
                usedeflation = true,
