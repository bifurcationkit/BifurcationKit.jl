...
let
    for accumulator in (Val(:Prod), Val(:Mean))
        ...
        _T = Float64
        deflationOp = DeflationOperator(2, dot, one(_T), [rand(2) for _ in 1:3]; accumulator)
        let
            _x0 = rand(2)
            _res = accumulator isa Val{:Prod}  ? 1 : 0
            _acc = if accumulator isa Val{:Prod} 
                * else
                 +
            end
            for r in deflationOp.roots
                _res = _acc(_res, deflationOp.α + norm(_x0 - r)^(-2deflationOp.power))
            end
            if accumulator isa Val{:Prod} 
                @test _res ≈ deflationOp(_x0) rtol = 1e-8
            else
        ...
        deflationOp2 = DeflationOperator(_T(2), BifurcationKit.CustomDist((u,v)->norm(u-v)), one(_T), deflationOp.roots)
        ...
        let
            n = 3
            deflationOp = DeflationOperator(2, dot, one(_T), [1 .+ 0.01rand(n) for _ in 1:3]; autodiff = true)
            rhs = rand(n)
            sol = rand(n)
