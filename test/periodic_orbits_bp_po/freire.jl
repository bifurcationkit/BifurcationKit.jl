...
begin
    for M in (1,15)
        ...
        @test_skip bp.nf.nf.a01 ≈ 1e-6    atol = 1e-5
        @test_skip bp.nf.nf.b11 ≈ 200     rtol = 1e-2
        @test_skip bp.nf.nf.b20 ≈ 4e-4    atol = 1e-2
        ...
        @test_skip ns.nf.type == :SubCritical
        @test_skip ns.nf.nf.b ≈ 68.8696641601362 + 288.2521772452238im rtol = 1e-2
        
        pd = get_normal_form(br_po_bp, 2; detailed = Val(true))
        @test pd.nf.type == :SuperCritical
        @test_skip pd.nf.nf.a ≈ -1254 rtol = 1e-2
        @test_skip pd.nf.nf.b3 ≈ 5548 rtol = 1e-2
...
begin
    for meshadapt in (false, true)
        br_po = continuation(br, 1, 
        ContinuationPar(br.contparams, ds = -0.001, dsmax = 0.01, tol_stability = 1e-4, p_min = -0.7), 
        Collocation(30, 4; jacobian = BK.DenseAnalyticalInplace(), meshadapt);
        δp = 0.001,
        )
        # plot(br, br_po)
        @test br_po.specialpoint[1].type == :bp
        ...
        @test_skip bp.nf.nf.a01 ≈ 1e-5  atol = 1e-5
        @test_skip bp.nf.nf.b11 ≈ 219   rtol = 1e-2
        @test_skip bp.nf.nf.b20 ≈ -4e-4 atol = 1e-2
        @test_skip bp.nf.nf.b30 ≈ 1657  rtol = 1e-2
        ...
        @test_skip ns_p.nf.nf.b/(imag(ns_p.nf.nf.b)) ≈ (-6.724846398058799 - 52.018420910967144im)/(- 52.018420910967144) rtol = 1e-1
        @test_skip ns_p.nf.type == :SuperCritical
...
begin
    br_po = continuation(br, 1, 
                ContinuationPar(br.contparams, ds = -0.001, dsmax = 0.01, tol_stability = 1e-4, p_min = -0.7), 
                Trapeze(M = 100; jacobian = BK.Dense());
                δp = 0.001, 
    )
