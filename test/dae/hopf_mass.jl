...
function hopf_nf(prob; start_with_eigen = Val(true))
    opts = ContinuationPar(dsmin = 0.001, dsmax = 0.02, ds = 0.01,
        p_max = 0.1, p_min = -0.3, detect_bifurcation = 3, nev = 2)
...
let
    ...
    daeprob = BK.DAEMassBifProblem(prob_ode, Matrix{Float64}(LA.I, 2, 2))
    ω, a, b = hopf_nf(daeprob; start_with_eigen = Val(false))
    @test ω ≈ 1 atol = 1e-9
    @test real(a) ≈ 1 atol = 1e-6
    @test imag(a) ≈ 0 atol = 1e-6
    @test b / 2 ≈ (-par_sl.c3 + im * par_sl.μ) atol = 1e-4
end
...
let
    ...
    prob_ode = BK.ODEBifProblem(Fsl2, zeros(2), par_sl, (@optic _.r))
    α = 2.0
    daeprob = BK.DAEMassBifProblem(prob_ode, α .* Matrix{Float64}(LA.I, 2, 2))
    ω, a, b = hopf_nf(daeprob; start_with_eigen = Val(false))
    @test ω ≈ 0.5 atol = 1e-9
    @test real(b) < 0 # supercritical
end

let
    # `start_with_eigen = Val(true)` is not supported for a DAE problem
    prob_ode = BK.ODEBifProblem(Fsl2, zeros(2), par_sl, (@optic _.r))
    daeprob = BK.DAEMassBifProblem(prob_ode, 2.0 .* Matrix{Float64}(LA.I, 2, 2))
...
let
    ...
    @test BK.getmassmatrix(daeI, zeros(2), par_sl) ≈ Matrix{Float64}(LA.I, 2, 2)
    # function (parameter dependent) mass matrix
    Bfun = (x, p) -> (p.r + 1) .* Matrix{Float64}(LA.I, 2, 2)
    daefun = BK.DAEMassBifProblem(prob, Bfun)
    @test BK.getmassmatrix(daefun, zeros(2), par_sl) ≈ (par_sl.r + 1) .* Matrix{Float64}(LA.I, 2, 2)
    # re_make replaces the mass matrix and forwards the usual keywords
    daeprob2 = BK.re_make(daeprob; M = 2 .* Matrix{Float64}(LA.I, 2, 2), u0 = [1.0, -1.0])
    @test BK.getu0(daeprob2) == [1.0, -1.0]
    @test BK.getmassmatrix(daeprob2, zeros(2), par_sl) ≈ 2 .* Matrix{Float64}(LA.I, 2, 2)
