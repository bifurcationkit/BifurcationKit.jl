...
### 2a. Secant predictor
This predictor is called **secant** and is parametrized by the option `tangentAlgo = SecantPred()`.  It is computed by $(dx, dp) = (z_1, p_1) - (z_0, p_0)$ and normalized by the norm $\|(x, p)\|^2_\theta = \frac{\theta}{length(x)} \langle x,x\rangle + (1 - \theta)\cdot p^2$. 
